...
# Match the entire get_journey function
...
async def get_journey(orig_lat: float, orig_lng: float, dest_lat: float, dest_lng: float):
    ...
    for retry in range(4):
        ...
        first_transit_idx = next((i for i, l in enumerate(legs) if l['type'] not in ['walk']), -1)
        if first_transit_idx != -1:
            first_transit = legs[first_transit_idx]
            try:
                ...
                if eta_res.get('success') and not eta_res.get('cached'):
                    ...
                    if not etas:
                        # Prune this line and retry
                        disabled_lines.add((first_transit['line'], first_transit['type']))
                        if not best_overall_route:
                            best_overall_route = {"legs": legs, "route_ids": list(route_ids_used)}
                        continue
                    
                    eta_val = etas[0].get('eta')
                    first_transit['live_eta'] = eta_val
                    walk_time = sum(l['time'] for l in legs[:first_transit_idx])
                    eta_mins = parse_time_str(eta_val)
                    if eta_mins != float('inf'):
                        wait_time = max(0, eta_mins - walk_time)
                        first_transit['wait_time'] = wait_time
